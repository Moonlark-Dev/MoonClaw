...
class Config(BaseModel):
    ...
    # 交互式命令执行工具配置
    interactive_exec_enabled: bool = True  # 是否启用交互式命令执行工具
    interactive_exec_timeout: int = 300  # 命令执行超时时间（秒）
    interactive_exec_max_output_length: int = 4000  # 最大输出长度
    interactive_exec_allowed_commands: list[str] | None = None  # 允许执行的命令白名单
    interactive_exec_blocked_commands: list[str] = ["rm", "del", "format"]  # 禁止执行的命令黑名单
